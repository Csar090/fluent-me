(function(){
  ...
  async function saveEnhanced(repeat){
    ...
    const prior=(await all()).find(x=>x.id===$('sessionId').value)||{};enhanceReview();const keep=$('retainAudio').checked,group=$('groupId').value||crypto.randomUUID(),attempt=Number($('attemptNo').value)||1;
    const row=Object.assign({},prior,{id:$('sessionId').value||crypto.randomUUID(),groupId:group,attempt:attempt,createdAt:prior.createdAt||new Date().toISOString(),title:$('title').value.trim(),context:$('context').value,audience:$('audience').value.trim(),duration:duration,transcript:$('transcript').value.trim(),timestampedTranscript:$('timestampedTranscript').value.trim(),segments:segments,reflection:$('reflection').value.trim(),tags:$('tags').value.trim(),metrics:lastMetrics,audio:keep?blob:null,audioType:keep&&blob?blob.type:''});
